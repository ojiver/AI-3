...
1. 學習目標
瞭解深度學習(Deep Learning)和神經網路(Neural Networks)的基本概念。
掌握使用 TensorFlow 進行進行數據處理、模型建構、訓練和評估的技巧。
學會進行數據正規化和模型參數的調整。
實作建構、訓練和評估深度學習模型。
提升問題解決和模型優化的能力。
2. 步驟說明
...
Model: “sequential”：表示我們所建立的模型是一個序列性模型（Sequential），這是最簡單的 Keras 模型類型，每一層就像是樂高積木一樣按順序一個接一個。
Layer (type)：顯示每層的類型，如 Flatten、Dense 等。
Output Shape：顯示每層的輸出形狀。例如
👉 flatten (Flatten) 是模型的第一層，將圖像從二維圖像（28×28像素）轉換為一維向量（784像素），輸出形狀是(32, 784)，其中 32 是批次大小（batch size），784 是展平後的特徵數量。
👉 dense (Dense) 是模型的第二層，也是一個全連接層（Dense），它有 128 個神經元。
👉 dense_1 (Dense) 則是模型的第三層，也是一個全連接層，它有 10 個神經元，用於輸出模型對 10 個類別的預測。
Param #：表示每層中訓練參數的數量。全連接層(Dense)的參數數量取決於前一層的輸出單元數和當前層的單元數。
👉 flatten 層的參數數量為0，因為它只做數據轉換，不包含任何需要訓練的參數。
👉 第一個 dense 層的參數數量是 100480，這是由 784（輸入單元）x 128（輸出單元）+ 128（bias 偏差值）計算得來的。
👉 第二個dense層的參數數量是1290，這是由 128（輸入單元）x 10（輸出單元）+ 10（bias 偏差值）計算得來的。
Total params：顯示模型總共的參數數量，這裡是101770，表示模型中所有層的參數加起來的總數。
Trainable params：顯示可訓練的參數數量，這裡與總參數數量相同，意味著模型中所有的參數都是可訓練的。
Non-trainable params：顯示不可訓練的參數數量，在這個模型中沒有不可訓練的參數。
model.summary() 這個命令對於確認模型結構和調校非常有用，可以讓我們直觀地看到模型各層配置以及它們是如何連接的。而另一種方法則是將這個神經網路架構視覺化如下，也提供讀者參考。
